...
将原始数据转换为将在Datatables中显示的值的过程在Datatables的属于称为**渲染**，并使用{% include href/Options.html param="columns.render" %}选项执行。

## 数据渲染 Data rendering

在Datatables中使用数据渲染器的主要优点是您可以修改输出数据而无需修改原始数据。{% include href/Options.html param="columns.data" %}方法可以用于获取和设置数据，但是设置操作会增加复杂性，因此建议{% include href/Options.html param="columns.data" %}选项仅用于指向数据的原始数据，只读，而{% include href/Options.html param="columns.render" %}选项作为渲染器转换数据。

{% include href/Options.html param="columns.render" %}可以以多种不同的方式使用：
...
使用{% include href/Options.html param="columns.render" %}是最常见的方法，因为它可以绝对控制将显示给最终用户的数据（这是常规的Javascript函数，因此您可以几乎可以对数据做任何操作）。

该函数传递三个参数：

- 1，{% include href/Options.html param="columns.data" %}指定的数据。如果{% include href/Options.html param="columns.data" %}是`null`，那这里也会返回`null`。
...
在我们Datatables中，如果我们希望有一个显示创建者全名的列，我们可以使用如下方式来连接字符串（特别注意，作为第一个参数传递，`creator`对象如何通过使用{% include href/Options.html param="columns.data" %}选项进行分配）
...
最后，要从`price`和`cost`字段创建毛利率列，我们可以使用函数来计算所需要的值，请注意，在这种情况下，{% include href/Options.html param="columns.data" %}为`null`，所以传递给{% include href/Options.html param="columns.render" %}方法的第一个参数也为`null`，但是第三个参数提供对原始数据源对象的访问，因此我们可以从第三个参数里继续使用数据：
...
格式化不那么常见的选择是作为一个字符串，它仅指向表中应使用的数据。这个操作与使用{% include href/Options.html param="columns.data" %}的方式类似，但是请记住，这样渲染只能访问到{% include href/Options.html param="columns.data" %}指向的数据，而不能访问整行的数据。
...
内置的渲染器辅助函数可以在`$.fn.dataTable.render`对象下访问。它们是函数（允许将选项传递给它们），应立即执行并将其结果分配给{% include href/Options.html param="columns.render" %}方法。这听起来可能有些复杂，但这意味着您将使用以下内容：
...
渲染器辅助函数只是附加到`$.fn.dataTable.render`的函数，使它可以从一个位置轻松访问。这些函数必须返回将与{% include href/Options.html param="columns.render" %}方法一起操作的函数。
