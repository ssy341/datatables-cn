...
返回的单元格是被选择单个列的{% include href/tags.html param="tfoot" %}第一行的单元格。如果你`tfoot`有多个行要操作，则必须使用{% include href/api/Tables.html param="table().footer()" %}方法获取`tfoot`元素下的节点，然后再使用标准的DOM/jQuery方法来操作节点。
...
#### 描述(Description):
获取被选择单个列`tfoot`元素下的{% include href/tags.html param="td" %}/{% include href/tags.html param="th" %}
     
#### 返回(Returns):
{% include href/type/DataTables.html param="node" %},{% include href/type/DataTables.html param="null" %}

所选元素的`tfoot`单元格节点，如果匹配的列没有`tfoot`元素，则为`null`。
--- 
    
## 例子(Example)

对第四列列值进行求和，并且将结果插入到`tfoot`元素上
