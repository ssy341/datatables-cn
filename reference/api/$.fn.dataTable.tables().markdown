...
请注意，这是一个静态的方法，不需要用API实例去掉用，通过`$.fn.dataTable`对象调用即可，并且随时可以执行这个方法，
甚至可以在生成表格之前调用这个方法。
...
#### 返回(Returns):
{% include href/type/type.Javascript param="array" %},{% include href/type/type.DataTables param="DataTables.Api"%}
