...
## 描述(Description)
DataTables 把行列的数据储存在内部索引里面，这个可以用来快速的执行排序和搜索等操作。当你使用{% include href/APIs.html param="row()" %}，{% include href/APIs.html param="column()" %}或者其他方法时，这个索引是很有用的。

并且这个方法提供了列数据索引还有可见列索引，同时还可以动态从DOM中添加和删除列。

使用 {% include href/APIs.html param="cell()" %} 方法获取的单元格返回的数据结构如下
...
- {% include href/APIs.html param="cells().indexes()" %}
- {% include href/APIs.html param="row().index()" %}
